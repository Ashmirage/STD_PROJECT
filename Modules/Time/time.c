...
volatile uint32_t sysTickUptime = 0; //ms计数,这个用于底层的延时函数,不能改动
...
void  SysTick_Configuration ( void )
{
    ...
    RCC_GetClocksFreq ( &rcc_clocks ); //获取CPU主频

    cnts = ( uint32_t ) rcc_clocks.HCLK_Frequency / TICK_PER_SECOND;
    cnts = cnts / 8; //计算1ms所需要的重装值
...
u32 systime_ms; //另一个ms计数值,这是用于业务逻辑,比如任务调度器里使用

void sys_time()
{
	systime_ms++;
}
u32 SysTick_GetTick(void)  //毫秒级别时间戳
{
	return systime_ms;
}
