...
//                  SPFD5408/1505/B505/C505/NT35310/NT35510/SSD1963/ILI9488等		    
...
// 注意背光引脚被改成了PB15,不要使用正点原子引出的BL脚, 除非修改代码
// 复位脚被改为了PE2, 不要使用正点原子引出的RST脚, 除非修改代码
// 其余的FSMC接线几乎固定,不要更改
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	LCD_SetCursor(x,y);	    
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0X9488)LCD_WR_REG(0X2E);//9341/6804/3510/1963/9488 发送读GRAM指令
	...
	opt_delay(2);	  
 	...
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X9488)	//9341/NT35310/NT35510/9488要分2次读出
 	{
		opt_delay(2);	  
		b=LCD_RD_DATA(); 
	...
	else if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X9488)return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//ILI9341/NT35310/NT35510/ILI9488需要公式转换一下
	else return LCD_BGR2RGB(r);						//其他IC
}			 
//LCD开启显示
void LCD_DisplayOn(void)
{					   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0X9488)LCD_WR_REG(0X29);	//开启显示
...
{	   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0X9488)LCD_WR_REG(0X28);	//关闭显示
...
void LCD_Scan_Dir(u8 dir)
{
	...
	if(lcddev.id==0x9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963||lcddev.id==0X9488)//9341/6804/5310/5510/1963/9488,特殊处理
	{
		switch(dir)
		{
...
{    
	...
	twidth=sx+width-1;
	theight=sy+height-1;
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X6804||(lcddev.dir==1&&lcddev.id==0X1963)||lcddev.id==0X9488)
	{
		LCD_WR_REG(lcddev.setxcmd); 
		LCD_WR_DATA(sx>>8); 
		LCD_WR_DATA(sx&0XFF);	 
  ...
  FSMC_NORSRAMInit(&FSMC_NORSRAMInitStructure);  //初始化FSMC配置
	...
	{	
		...
		lcddev.id|=LCD_RD_DATA();  	//读取41或者88	   			   
 		if((lcddev.id!=0X9341)&&(lcddev.id!=0X9488))		//非9341也非9488,尝试是不是6804
		{	
 			LCD_WR_REG(0XBF);				   
			lcddev.id=LCD_RD_DATA(); 	//dummy read 	 
