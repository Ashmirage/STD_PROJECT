...
#define DFT_SCAN_DIR U2D_R2L// L2R_U2D  //默认的扫描方向   
...
void LCD_Draw_Circle(uint16_t x0,uint16_t y0,u8 r);						 			//画圆
...
void LCD_ShowChar(uint16_t x,uint16_t y,u8 num,u8 size,u8 mode);						//显示一个字符
void LCD_ShowNum(uint16_t x,uint16_t y,u32 num,u8 len,u8 size);  						//显示一个数字
void LCD_ShowxNum(uint16_t x,uint16_t y,u32 num,u8 len,u8 size,u8 mode);				//显示 数字
void LCD_ShowString(uint16_t x,uint16_t y,uint16_t width,uint16_t height,u8 size,u8 *p);		//显示一个字符串,12/16字体
